...
// 3. Item three
// And so on.
var orderedListNumber: { [key: string]: any } = {},
...
const StyleItems: { [key: string]: Object } = {
  // BLOCK LEVEL
  "unordered-list-item": {
    open: function () {
      return "- ";
  ...
  "ordered-list-item": {
    open: function (block: any, number = 1) {
      return `${number}. `;
    },
...
// Both the open and close methods must exist, even if they simply return an empty string.
// They should always return a string.
const EntityItems: { [key: string]: Object } = {
  LINK: {
...
function isEmptyBlock(block: any) {
  return (
    block.text.length === 0 &&
...
function renderBlock(
  ...
  var markdownStyleCharactersToEscape: any[] = [];
  ...
  const compareTagsLastCloseFirst = (a: any, b: any) =>
  ...
  Array.from(block.text).some(function (character: any, characterIndex) {
    ...
    reverse(openTags).forEach(function (tag: any) {
    ...
    var inlineTagsToOpen = block.inlineStyleRanges.filter(
      (tag: any) => tag.offset === characterIndex
    );
 ...
 * @return {String} markdown string
 **/
function draftToMarkdownTest(rawDraftObject: any, options?: any) {
  options = options || {};
  var markdownString = "";
  rawDraftObject.blocks.forEach(function (block: any, index: any) {
...
export default draftToMarkdownTest;
