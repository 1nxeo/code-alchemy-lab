...
const DefaultBlockTypes = {
  ...
  blockquote_open: function (item?: any) {
    return {
      type: "blockquote",
      text: "",
    };
  ...
  heading_open: function (item: { [key: number | string]: string }) {
    let type =
      "header-" +
      {
        1: "one",
...
const DefaultBlockEntities = {
  link_open: function (item: { [key: string | number]: string }) {
    return {
      type: "LINK",
      mutability: "MUTABLE",
      data: {
...
function parseInline(inlineItem: any, BlockEntities: any, BlockStyles: any) {
  var content = "",
    blockEntities: { [key: string]: string } = {},
    blockEntityRanges: any[] = [],
    blockInlineStyleRanges: any[] = [];
  inlineItem.children.forEach(function (child: any) {
    if (child.type === "text") {
...
interface RemarkableOptionsType extends Remarkable.Options {
  enable: any;
  disable: any;
}
function markdownToDraftTest(
  string: any,
  options: {
    remarkablePreset: "commonmark" | "full" | "default";
    remarkableOptions: RemarkableOptionsType;
    blockTypes: { [key: string | number]: any };
    blockEntities: { [key: string | number]: any };
    preserveNewlines: { [key: string | number]: any };
    blockStyles: { [key: string | number]: any };
  ...
  const remarkableOptions =
    typeof options.remarkableOptions === "object"
      ...
      : null;
  const md = new Remarkable(remarkablePreset, remarkableOptions!) as any;
  ...
  var blocks: any[] = []; // blocks will be returned as part of the final draftjs raw object
  ...
  var currentListType: any = null; // Because of how remarkable's data is formatted, we need to cache what kind of list we're currently dealing with
...
export default markdownToDraftTest;
